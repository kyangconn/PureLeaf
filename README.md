...
```
apps/                    ← 应用层（Qt Quick / HarmonyOS ArkTS）
  desktop-qml/           ← Qt 6 桌面应用
...
|------|------|
| CMake | ≥ 3.21 |
| Qt 6 | ≥ 6.5（Core、Gui、Qml、Quick、QuickControls2、Svg） |
...
# 运行
out\build\x64-debug\apps\desktop-qml\desktop-qml.exe
```
...
# 运行
out/build/linux-debug/apps/desktop-qml/desktop-qml
```
...
| 选项 | 默认值 | 说明 |
|------|--------|------|
| `PURELEAF_BUILD_QML` | `ON` | 构建 Qt Quick 桌面应用 |
...
├── apps/desktop-qml/           # Qt Quick 桌面应用源码
│   ├── main.cpp                # 入口
│   ├── Main.qml                # 主窗口（QWindowKit 无边框）
